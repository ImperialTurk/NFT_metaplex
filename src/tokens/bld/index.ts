...
const TOKEN_IMAGE_PATH = `tokens/bld/assets/${"erlik.jpg"}`;
...
async function createBldToken(
  ...
  payer: web3.Keypair
    ...
    // Read image file
    const imageBuffer = fs.readFileSync("C:\\face_nft\\tokens\\bld\\assets\\erlik.jpg");
    const file = toMetaplexFile(imageBuffer, "erlik.jpg");
    ...
    const { uri } = await metaplex
        ...
        .uploadMetadata({
        name: "Erlik Khan",
        description: "Erlik Khan is the god of death and the underworld, sometimes referred to as Tamag (hell) in Turkic mythology. Er (or yer) means Earth, in the depths of which Erlik lives in.From the underworld, Erlik brings forth death, plague and evil spirits to torment humans and take their souls into his realm.",
        image: imageUri,
        })
        .then();
    ...
    fs.writeFileSync(
        "C:\\face_nft\\tokens\\bld\\cache.json",
        JSON.stringify({
